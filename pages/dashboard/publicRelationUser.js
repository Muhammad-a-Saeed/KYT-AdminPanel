...
const PublicRelationUser = () => {
  ...
  const getAllData = async (pageNumber = 1, size = 10) => {
    ...
    try {
      ...
      const resp = await getAllPublicRelationData(getToken, pageNumber, size);
      if (resp) {
        setCurrentItems(resp?.prRequests);
        setData(resp);
      }
    } catch (error) {
      setLoading(false);
  ...
  const handleDropdownAction = async (userId, status) => {
    ...
    const getToken = auth_user.token;
    setLoading(true);
    await editPublicRelationData(getToken, editBody, userId)
      .then(async (res) => {
    ...
    >
      ...
      <div>
        <div className="flex text-[#4D515A] text-sm font-medium">
          Dashboard <Dashboard_Chevron /> Public Relation User{" "}
          <Dashboard_Chevron />
        </div>
